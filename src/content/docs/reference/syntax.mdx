...
description: This page is a demonstration of the various markdown syntax supported by Starlight.
---
...
See the [frontmatter reference](https://starlight.astro.build/reference/frontmatter/) for all available fields and how to add custom fields.
...
You can [link to another page](https://starlight.astro.build/getting-started/).
...
Starlight is configured to automatically use your page title as a top-level heading and will include an "Overview" heading at top of each page's table of contents.
...
Starlight provides a custom Markdown syntax for rendering asides.
Aside blocks are indicated using a pair of triple colons `:::` to wrap your content, and can be of type `note`, `tip`, `caution` or `danger`.
...
- [i18n support](https://starlight.astro.build/guides/i18n/)
...
You can indicate the programming language being used after the opening backticks.
...
Code block rendering can be configured using Starlight's [`expressiveCode` configuration option](https://starlight.astro.build/reference/configuration/#expressivecode).
...
Starlight supports all other Markdown authoring syntax, such as lists and tables.
...
And standard ordered lists:

1. First Item
2. Second Item
3. Third Item

And standard unordered lists:
